        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            >
                ...
                >
                    ...
                    <span v-else
                        >{{ isEdit ? "Update" : "Save" }} Adjustment</span
                    >
                </button>
            </div>
        </form>
...
const isEdit = computed(() => !!props.adjustment);
...
const isFormValid = computed(() => {
    return (
        ...
        formData.value.adjustment_date &&
        formData.value.details.length > 0 &&
        formData.value.details.every(
            (detail) =>
                detail.product_id &&
                detail.reason &&
...
const calculateDetailDifference = (index) => {
    ...
    console.log(
        `[AdjustmentFormModal] Calculated difference for index ${index}:`,
        {
            actualQty,
            systemQty,
...
const handleSubmit = () => {
    ...
    const submitData = {
        adjustment_date: formData.value.adjustment_date,
        ...
        details: formData.value.details.map((detail) => ({
            ...
            actual_quantity: parseFloat(detail.actual_quantity),
...
// Watch for adjustment changes (edit mode)
watch(
    () => props.adjustment,
    (newVal) => {
        if (newVal) {
            console.log(
                "[AdjustmentFormModal] Edit mode - adjustment data:",
                newVal
            );

            formData.value = {
                adjustment_date: newVal.adjustment_date
                    ? new Date(newVal.adjustment_date)
                          .toISOString()
                          .split("T")[0]
                ...
                details:
                    newVal.details && newVal.details.length > 0
                        ? newVal.details.map((detail) => {
                              ...
                              console.log(
                                  `[AdjustmentFormModal] Processing detail for product ${detail.product_id}:`,
                                  {
                                      systemQty,
                                      actualQty,
                                      diffQty,
            ...
            console.log(
                "[AdjustmentFormModal] Form data populated:",
                formData.value
            );
        }
    },
